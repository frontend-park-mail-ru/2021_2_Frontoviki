...
export function CreateModal() {
    ...
    const patterns = {
        ...
        password: /^[a-z\d]{5,20}$/i, ///^(?=.*[0-9])(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z!@#$%^&*]{8,}$/i,
        telephone: /^\d{11}$/,
        slug: /^[a-z\d-]{8,20}$/i,
    };
    ...
    promtUsernameAlrdyExist.innerHTML = 'Пользователь с таким именем уже существует';
    ...
    promtUsernameInvalidName.innerHTML = 'Имя пользователя должно быть от 5 до 20 символов';
    ...
    promtEmailInvalidName.innerHTML = 'Неверный формат емайла (пример: exam@ple.com)';
    ...
    passwordR.addEventListener('keyup', (e) => {
        e.target.classList = '';
        // очищаю класс show у server-invalid
        const promt = e.target.nextSibling.firstChild;  // input->div->p
        promt.classList.remove('show');
    ...
    promtPasswordInvalid.innerHTML = 'Пароль должен состоять из букв, цифр и спец символов, длинной от 8 символов';
    ...
    regForm.addEventListener('submit', (e) => {
        ...
        let valid = validate(nameR, patterns[nameR.attributes.name.value]) &&
                    validate(emailR, patterns[emailR.attributes.name.value]) &&
                    validate(passwordR, patterns[passwordR.attributes.name.value]);
